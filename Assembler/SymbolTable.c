...
// Creates pointer to symbol_table_entry_t equal to NULL, which can be thought of
// as the "root" object for the table. All other functions will take a pointer to this
// pointer to manipulate the table. From the documentation:
// "The reason it’s necessary to deal with a pointer to the hash pointer is simple:
// the hash macros modify it (in other words, they modify the pointer itself not just what it points to)."
...
void SymbolTable__addEntry(symbol_table_entry_t **symbol_table, const char *symbol, size_t symbol_length, uint16_t value)
{
...
uint16_t SymbolTable__getAddress(symbol_table_entry_t **symbol_table, char *symbol)
{
  symbol_table_entry_t *s;
  HASH_FIND_STR(*symbol_table, symbol, s);
